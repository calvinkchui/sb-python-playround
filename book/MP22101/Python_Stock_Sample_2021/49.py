...
#取得大單時間以及價格
BigTime = [ datetime.datetime.strptime(line[0],"%Y/%m/%d %H:%M:%S.%f") for line in Data if int(line[3]) > big_limit ]
BigPrice = [ float(line[2]) for line in Data if int(line[3]) > big_limit ]
...
ax.plot_date(BigTime, BigPrice, 'ro')
