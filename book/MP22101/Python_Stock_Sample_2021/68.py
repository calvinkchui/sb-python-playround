...
# 策略判斷
for rnum in range(0,len(Data)):
    Time=Data[rnum][0]
    Price=float(Data[rnum][2])
    Qty=int(Data[rnum][3])
    DnPrice=float(Data[rnum][5])
    UpPrice=float(Data[rnum][6])
    ...
    if Index == 0:
        # 多單進場判斷
        if BuyPower > SellPower + powerdiff :
            Index=1
            OrderTime=Time
            OrderPrice=Price
        # 空單進場判斷
        elif BuyPower + powerdiff < SellPower :
            Index=-1
            OrderTime=Time
            OrderPrice=Price
    # 多單出場判斷
